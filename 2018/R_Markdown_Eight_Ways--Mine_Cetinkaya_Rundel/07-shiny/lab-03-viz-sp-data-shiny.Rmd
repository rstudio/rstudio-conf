...
Next we explore potential clustering of Denny's and La Quinta locations within certain states. To do so we'll `filter` the data for a particular state and visualize the locations of the two establishments. Before writing our own code to create these visualizations, let's play with an interactive map a little bit.

```{r shiny, eval=TRUE, echo=FALSE, message=FALSE}
# Load packages -----------------------------------------------------
library(shiny)
#library(tidyverse)

# Load data ---------------------------------------------------------
dn_lq_us <- read_csv("data-shiny/dn_lq_us.csv")
us <- map_data("state")

# Figure out list of states to list ---------------------------------
states <- dn_lq_us %>%
  distinct(state) %>%
  arrange(state) %>%
  pull()

# Denny's and La Quinta colors --------------------------------------
lq_col <- "#004827"
dn_col <- "#FFDE23"
...
ui <- fluidPage(
   ...
   # Application title
   titlePanel("Is La Quinta next to Denny's?"),
...
server <- function(input, output) {
   ...
   output$scatterplot <- renderPlot({
     if(input$zoom){
       ...
       ggplot(dn_lq_us, mapping = aes(x = longitude, y = latitude)) +
         geom_polygon(data = us, aes(x = long, y = lat, group = group), fill = NA, color = "lightgray") +
         geom_point(alpha = 0.5, col = "gray") +
         geom_point(data = filter(dn_lq_us, state == input$state),
                    mapping = aes(x = longitude, y = latitude, color = establishment), alpha = 0.8) +
         scale_color_manual(values = c(dn_col, lq_col)) +
         labs(x = "Longitude", y = "Latitude", color = "Establishment") +
         theme_bw()
     }
